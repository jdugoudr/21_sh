...
/*   Updated: 2019/06/15 14:42:46 by jdugoudr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ambigous_redirect(t_ast *token)//fonction a supprimer c'est juste pour test
{
	while (token->next)
	{
...
static int		sub_parser(char *line, t_ast **token_head, t_ast **ast_root)
{
	...
	else if (!(tmp = look_arg(*token_head)) || expansion_tok(*token_head)
